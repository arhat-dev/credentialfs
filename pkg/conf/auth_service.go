...
func (c *AuthServiceConfig) UnmarshalJSON(data []byte) error {
	...
	c.Name, c.Config, err = unmarshalAuthServiceConfig(m)
	if err != nil {
		return err
	}
...
func unmarshalAuthServiceConfig(m map[string]interface{}) (name string, config interface{}, err error) {
	n, ok := m["name"]
	if !ok {
		return
	}
	...
	name, ok = n.(string)
	if !ok {
		err = fmt.Errorf("auth service name must be a string")
		return
	}

	config, err = security.NewAuthorizationHandlerConfig(name)
	if err != nil {
		return
	}
